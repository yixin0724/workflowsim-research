...
```bash
mvn verify      # 完整门禁（核心 + 实验，单元 + 集成 + 覆盖率棘轮），约 1 分钟；
                # 各模块 Tests run 合计 446（290+75+27+54），Failures/Errors 全 0（2026-09-16 R8 审计轮实录）
```
...
- `SimulationConfig`：单次运行的不可变配置，含输入（可多工作流 + 错峰提交时刻）、算法、存储、随机种子、故障、开销、deadline、成本和数据移动模型。
