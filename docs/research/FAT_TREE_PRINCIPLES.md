...
- 每条链路对"当前占用它的所有流"做公平共享（capacity / n，与 R2 同语义），
  流的瞬时速率 = **路径上所有链路份额与端点份额的最小值**。
- 字节余量按事件时间线性积分；竞争者加入/退出时速率变化被逐段吸收
  （R2 `TransferContentionEngine` 既有机制的直接推广：争用域从"VM 端点"
  推广为"任意链路对象"）。
- **VM 端点 bw（R2 语义）与链路 bw 同时生效、取最小**——两代模型语义平滑
  衔接，R2 成为"无内部链路争用"的特例。
