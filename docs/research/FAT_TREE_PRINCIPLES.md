...
> **清理轮注记（R11）**：本笔记精读的仓库内 CloudSim 3.0.3 `org.cloudbus.cloudsim.network.datacenter`
> 包（及 `cloudsim.power` 子树等 vendored 死代码）已在死代码清理轮删除——§0 结论摘要第 4 条
> "不可复用"的结论即删除依据。本文作为历史研读记录完整保留，文中源码行号引用不再可解析。
