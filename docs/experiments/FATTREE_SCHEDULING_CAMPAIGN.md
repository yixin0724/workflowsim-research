...
> 修订：F1 链路容量单位修复（声明值 ÷8 被解释）后重录 campaign 工件与全部
> 黄金值；声明链路带宽 == 端点带宽时链路层退化为精确 no-op 成为本设计的
> 一等预期（见 §2.4 R8 附注）。
...
噪声级交叉（R8 重录 campaign 实测：120 组合中 1 处，cybershake-n30 ×
RANDOM 的 R2 低于 V1 约 3.7×10⁻⁶）。因此弱单调性在本 campaign 中作为
**诊断**如实报告（JSON/MD 工件含违反清单与最大相对偏差），仅在已实证成立
的 HEFT 论文例子集上做严格断言（golden IT）。R8 单位修复后的一个直接推论：
当声明链路带宽 ≥ 端点带宽时，min 恒等于端点份额，R6 与 R2 **逐位相等**，
弱单调性以等号平凡成立（campaign 实测 10 DAG × 4 规划器全零差值）。
...
状态。R6 运行使用 k=4 满配拓扑、链路 1 MB/s。**R8 预期修订**：链路声明值
与 VM 端点带宽（1 MB/s）相等，单位修复后链路层永不束缚单流，R6 与 R2
逐位相等是该装置的必然结果（重录实测确认，见结果文档 §0.1）；若要链路
争用可见，须声明链路带宽 < 端点带宽（结果文档 §0.8 再标定建议）。
...
| A4 链路带宽 | k=4 满配、链路 10 MB/s | 链路远宽于端点时是否收敛回 R2 式端点主导。**R8 注**：单位修复后该声明值是真 10×（端点 1 MB/s 的 10 倍；修复前 ÷8 使"10 MB/s"实际只有 1.25 MB/s，标签失真）；基线链路 1.0 == 端点后 A4 与基线同为非束缚，轴退化恒等 |
...
配对差值全为零）。首轮曾报告"只有带宽比轴（A4）有效并精确收敛回 R2 值"，
R8 审计证明那是链路 ÷8 旧物理的产物（基线链路被压到端点 1/8，A4 ×10 后
恰好越过端点）；单位修复后 **A4 与基线同为非束缚，带宽轴也退化为恒等**。
为激活结构轴判别力，campaign 增加 **4 主机结构块**：同 6 变体（A5 改为两个
同 edge 主机对 {0→0,1→0,2→2,3→2}），4 主机允许不共享端点的交叉流
（如 0→2 与 1→3），4 主机平台 = 4 VM，静态映射同构扩展。路由层契约由
`FatTreeTopologyTest` 结构轴测试锁定：A1 下交叉流确实共享 core 链路、
A2 单 Pod 化、A5 缩短同 edge 路径——拓扑参数确实重塑路由与共享格局。

**R8 附注（全轴退化，2026-09-16 重录实测）**：单位修复后基线链路 1.0 MB/s
== 端点带宽，链路层对任何流都不构成约束，min(端点份额, 链路份额) = 端点份额
恒成立——3 主机与 4 主机两个块的全部 6 变体（含 A4）与基线逐位恒等，
R6 ≡ R2。"恒等是动态层的真实现象而非拓扑参数失效"的证据链随之更新：
路由层仍由 `FatTreeTopologyTest` 锁定；动态层原 A4 有效性证据改由 simulator
慢链路探针承担（`FatTreeContentionIntegrationTest`：链路 0.25 MB/s < 端点时
makespan 588.1 > R2 284.1；对称场景 R6 与 R2 逐位相等 284.1）。两个块的
恒等结果作为退化边界如实保留在工件中；恢复拓扑轴判别力须再标定声明链路
带宽 < 端点（结果文档 §0.8，未执行）。
...
4. 关键不变量与抽样黄金值由 IT 锁定：simulator 模块
   ...
   `FatTreeCampaignGoldenIntegrationTest`（6 IT，R8 按重录 campaign 工件
   更新：campaign 配置黄金值 R6 列 = R2 列、逐 DAG 排名翻转、弱单调诊断
   噪声级、确定性双跑、3 主机敏感性全轴退化恒等、4 主机结构块全退化含
   A4 == 基线）。
