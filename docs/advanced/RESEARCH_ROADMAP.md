...
> bug（声明 1 MB/s 实际按 0.125 MB/s 运行）。修复后同装置实测 284.1——对称
> 供给下链路层不束缚单流，R6 ≡ R2；契约改为 284.1 对称等价锁 + 588.1 慢链路
> （0.25 MB/s）探针锁。详见 `docs/advanced/COMPREHENSIVE_AUDIT_R8.md` §3.3 N-1/N-2。
