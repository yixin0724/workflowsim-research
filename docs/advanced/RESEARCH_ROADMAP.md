...
> **R10现状**：当前平台已修正CPOP/max-min语义，提供v4完整证据、网络受限多规模多种子研究和统一离线报告入口。下文R1-R7数字为历史记录，不替代[R10验收](PLATFORM_UPGRADE_R10.md)。Java仅负责RL环境与策略适配，不建设模型训练。
...
本平台是**受控抽象环境**下的工作流调度模拟器，支持无争用、端点争用与Fat-tree流体争用模型。这一边界适合做算法
...
> **R8 审计注记（2026-09-16）**：上述 1032.1 产自 FatTree 链路容量 ÷8 单位
> bug（声明 1 MB/s 实际按 0.125 MB/s 运行）。修复后同装置（simulator 论文
> fixture，链路 1.0 == 端点）实测 284.1——对称供给下链路层不束缚单流，
> R6 ≡ R2；契约改为 284.1 对称等价锁 + 588.1 慢链路（0.25 MB/s）探针锁。
> campaign 装置随后经用户授权再标定为链路 0.125 MB/s（端点 1/8）恢复束缚
> 区间（R6 > R2 真实成立，见 R7 节注记与结果文档）。详见
> `docs/advanced/COMPREHENSIVE_AUDIT_R8.md` §3.3 N-1/N-2。
...
- 契约测试：`FatTreePlannerCompatibilityIntegrationTest`（simulator，3 IT，
  ...
  换冠、确定性、敏感性结构轴恒等与 A4 收敛、4 主机结构块退化与带宽效应）；
  `FatTreeSchedulingCampaignExecutorTest`（10 单测：纯逻辑汇总/工件序列化）。

> **R8 注记（2026-09-16，两步）**：① 审计重录——F1 单位修复后按原声明
> （链路 1.0 == 端点 1.0 MB/s）重跑，实测对称供给下 R6 ≡ R2 全退化，该
> 退化降为边界物理事实（审计报告 §3.3 N-2）；② 用户授权再标定——链路基线
> 0.125 MB/s（端点 1/8，恰恢复修复前实际物理区间）、A4 1.25 MB/s（真 10×，
> > 端点非束缚），13:42 全量重跑，束缚与判别力恢复。决定性交叉验证：再标定
> R6 列与修复前黄金值**逐位相等**（5738.1/5854.1/7206.1/7262.1；结构块
> 5718.1/5574.1/5186.1/5168.1），证明再标定精确恢复修复前物理并验证 F1
> 修复语义；R2/V1 列因 F2 修复有微漂移。下列验收数字为再标定重跑实测。

**验收实测**（全部数字来自 R8 再标定重跑 campaign 正式运行）：
...
- 平均排名三模型下均 HEFT < CPOP < PSO < RANDOM——campaign 配置下平均
  ...
  方向在 2 DAG × 2 模型上一致）；inspiral-n50 出现 R2/R6 第一名分化
  （R2 CPOP、R6 HEFT，噪声级幅度，链路层独立贡献的直接证据）；
- 配对 Wilcoxon：HEFT vs PSO 的优势被争用侵蚀到不显著（V1 p=0.0371 →
  R2/R6 p=0.0645）；R6−R2 中位差 HEFT +103.5 s（p=0.0059）/ CPOP +104.0 s
  （p=0.0078）/ RANDOM +441.8 s（p=0.0039）/ PSO +34.5 s（p=0.1289，不
  显著）——链路级争用显著且对 RANDOM 惩罚最大、对 PSO 相对温和；
- 敏感性：结构轴（k/超收敛/放置）在 3 与 4 主机平台上全部恒等（配对差值
  全为零；路由单测证明参数确实重塑共享格局，恒等是端点共享的平台物理而非
  带宽参数问题）；带宽比轴恢复判别力——A4（链路 1.25 MB/s > 端点）在
  3 主机敏感性精确收敛回主矩阵 R2（5195.1/5205.1），在 4 主机结构块严格
  降低 makespan（8/10 DAG，p=0.0078/0.0156）；
