...
> （6 组交叉、24 次端到端实跑 + 统一报告不变量校验器：完成性、指标内部自洽、
> 任务区间划分、流时定义、VM 汇总闭合），补齐三个既有覆盖缺口：① PSO 首次端到端
> 经 `SimulationRunner` 实跑；② 全部 6 个在线调度器 × 错峰到达（含 `DATA`+LOCAL）；
> ③ RL 轨道 × 错峰到达。全部通过；无功能性缺陷。同时修正 README 语义表中 R2
...
> （约束超集性质：≥ R2 黄金值 284.1 > 无争用 190.1；**R8 审计注记：该 1032.1 产自 FatTree ÷8 单位 bug，修复后契约改为对称等价 284.1 == R2 + 慢链路探针 588.1，详见 COMPREHENSIVE_AUDIT_R8.md §3.3**）；覆盖率实测
