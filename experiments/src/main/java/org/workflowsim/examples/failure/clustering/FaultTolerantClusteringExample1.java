...
/**
 * 使用动态聚类处理任务聚类故障的历史教学示例。
 *
 * <p>它直接操作历史故障与聚类全局状态，用于解释机制组合；故障分布和恢复效果只表示
 * 本场景的模型假设，不能作为标准研究证据或真实平台可靠性预测。</p>
 *
 * @author Weiwei Chen
...
public class FaultTolerantClusteringExample1 extends FaultTolerantSchedulingExample1 {
    ...
    // ======================== 静态示例入口 ========================
    /**
     * 运行单数据中心、单存储的历史容错聚类场景。
     */
    public static void main(String[] args) {

        try {
            ExampleCliSupport.begin("fault-clustering-dc");
            ...
            /* 容错参数。 */
            /**
             * MONITOR_JOB 按任务深度归类记录，MONITOR_VM 按 VM 标识归类，
             * MONITOR_ALL 汇总全部记录，MONITOR_NONE 不记录失败。
             */
            ...
            throw ExampleCliSupport.failure("fault-clustering-dc", e);
        } finally {
